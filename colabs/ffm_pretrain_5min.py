...
SEQ_LEN       = 64         # 64 bars × 5min = ~5.3 hours of market context
...
# Always fresh clone — no stale cache
...
# Check that your CSVs are where you said they'd be.
...
if not data_files:
    ...
    print(f"   Make sure your 5-min CSVs are at this path.")
    print(f"   Expected format: ES_5min.csv, NQ_5min.csv, GC_5min.csv, etc.")
...
# Saves as parquet for fast loading. **Run once** (or when you change label logic).
...
#   ✅ Sentinel-aware accuracy: structure labels skip ignore_index=-100 bars
...
MIN_TASK_ACC    = 0.10
MAX_MAJORITY    = 0.95
LABEL_SENTINEL  = -100  # CrossEntropyLoss ignore_index for low-confidence structure bars
...
# structure has 2 classes (bullish / bearish); conflicting bars → sentinel (-100)
TASK_NUM_CLASSES = {"regime": 4, "volatility": 4, "structure": 2, "range": 5}


# =========================================================
# FIX: Correct temporal train/val split
# =========================================================
# BUG in temporal_train_val_split: it splits bars correctly but then
# creates non-overlapping sequences only from the val bar window,
# producing far fewer val sequences than expected (~2,100 instead of ~118,000).
# This fix splits bars at the cutoff, then creates FFMDataset
# from each split independently — giving the correct ~VAL_RATIO of sequences.
# =========================================================
def fixed_temporal_split(features_df, labels_df, val_ratio, seq_len):
    """
    Correct temporal split that preserves expected val sequence count.
    Splits on bars at (1 - val_ratio) cutoff, then creates FFMDataset
    from each split. Val sequences ≈ val_ratio × total sequences.
    """
    n_bars = len(features_df)
    cutoff = int(n_bars * (1 - val_ratio))
    # Ensure val window has at least seq_len bars
    cutoff = min(cutoff, n_bars - seq_len - 1)

    train_features = features_df.iloc[:cutoff].reset_index(drop=True)
    train_labels   = labels_df.iloc[:cutoff].reset_index(drop=True)
    val_features   = features_df.iloc[cutoff:].reset_index(drop=True)
    val_labels     = labels_df.iloc[cutoff:].reset_index(drop=True)

    train_ds = FFMDataset(train_features, train_labels, seq_len=seq_len)
    val_ds   = FFMDataset(val_features,   val_labels,   seq_len=seq_len)
...
def check_collapse(preds_counter, num_classes, task_name):
    ...
    for cls, count in preds_counter.items():
        rate = count / total
        if rate > MAX_MAJORITY:
            return True, f"class {cls} = {rate:.0%}"
    return False, ""


def task_accuracy(preds, labels, sentinel=None):
    """Accuracy that skips sentinel-labeled rows (e.g. structure ignore_index=-100)."""
...
# =========================================================
# LOAD DATA — using fixed_temporal_split
# =========================================================
...
for feat_path in feature_files:
    ...
    # ✅ Using fixed split — replaces buggy temporal_train_val_split
...
# ── Sanity check: val must be close to VAL_RATIO ──
...
if actual_val_pct < 0.05:
    raise RuntimeError(
        f"Val set is only {actual_val_pct:.1%} of total sequences — "
        f"fixed_temporal_split may have failed. "
        f"Check that FFMDataset accepts the split DataFrames correctly."
    )
...
optimizer = torch.optim.AdamW(
    model.parameters(),
    lr=LEARNING_RATE,
    weight_decay=0.05,   # stronger regularization for pretraining
)
...
with open(ckpt_dir / "train_args.json", "w") as f:
    json.dump({
        ...
        "val_split_fix": "fixed_temporal_split — bars split at cutoff, sequences from each split",
    }, f, indent=2)
...
def evaluate(model, loader, device):
    ...
    for batch in loader:
        ...
        for task in TASK_NAMES:
            ...
            task_correct[task] += correct
            task_total[task] += total
            # Only record non-sentinel predictions for collapse detection
            if sentinel is not None:
                mask = labels != sentinel
...
for epoch in range(1, EPOCHS + 1):
    ...
    if len(val_loss_history) >= STABLE_EPOCHS:
        ...
        is_stable = loss_std < 0.01 and status_level in ("ok", "slt")
    else:
        loss_std = 0.0
        is_stable = False

    if is_stable and ratio <= MAX_RATIO:
        stable_counter += 1
    else:
        stable_counter = 0
    ...
    for task in TASK_NAMES:
        ...
        collapsed, reason = check_collapse(
            val_pred_counts[task], TASK_NUM_CLASSES[task], task
        )
        if collapsed:
    ...
    train_acc_str = " ".join(
        f"{t[0].upper()}:{train_acc[t]:.3f}" for t in TASK_NAMES
    )
...
# Plot loss and per-task accuracy.
...
# Quick test that the trained backbone loads and produces embeddings.
...
print(f"   Input:  ({batch_size}, {seq_len}, {num_feat})")
